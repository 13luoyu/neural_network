# glove模型损失函数为
# h(xij)(ujvi + bi + cj - logxij)^2
# h(x)知道
# xij是多重集Ci中元素j的重数
# uj和vi分别是词j的上下文表示和词i的中心词表示
# bi是中心词偏置
# cj是上下文词偏置，这两个是模型参数

# ui,vi,bi,ci都是模型参数，xij是要生成的数据
import collections
import math
import os
import random
...
def batchify(data):
    ...
    for center, context, negative in data:
        ...
        for j in context:
            dic[j] += 1
        for j in context:
            local_xij.append(dic[j])
        local_xij += [0] * (max_len - len(context))
        xij.append(local_xij)
    return torch.tensor(centers).reshape((-1,1)), torch.tensor(contexts_negatives),\
        torch.tensor(masks), torch.tensor(labels), torch.tensor(xij)

x_1 = (1, [2, 2], [3, 3, 3, 3])
x_2 = (1, [2, 2, 2], [3, 3])
batch = batchify((x_1, x_2))

names = ['centers', 'contexts_negatives', 'masks', 'labels', 'xij']
